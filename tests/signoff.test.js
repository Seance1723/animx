...
export function run() {
  try {
    console.log('[Tests] Running AnimX v3.42.0 Final Sign-Off verification...');

    // 1. Version
    assert.strictEqual(AnimX.version, '3.42.0', 'Version must be 3.42.0');
    assert.strictEqual(AnimX.versionInfo().version, '3.42.0', 'versionInfo version must be 3.42.0');
    assert.strictEqual(
    ...
    assert.strictEqual(pkg.version, '3.42.0');
    assert.ok(pkg.main);
