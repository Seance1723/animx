...
export function run() {
  try {
    console.log('[Tests] Running AnimX v3.36.0 RC Final Verification checks...');

    // 1. Version Asserts
    assert.strictEqual(AnimX.version, '3.36.0', 'Global version must be exactly 3.36.0');
    assert.strictEqual(AnimX.versionInfo().version, '3.36.0', 'versionInfo version must be exactly 3.36.0');
    assert.strictEqual(AnimX.versionInfo().release, 'Final Release Candidate QA and v4.0.0 Preparation', 'Release metadata must match milestone');
