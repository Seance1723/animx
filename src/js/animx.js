...
import { elementPresets } from './presets/element-presets.js'; // v3.37.0
...
// Old CMS API (v1.7) replaced by Advanced CMS API (v3.37.0)
...
import { cmsRecipes312 } from './cms/cms-recipes-v3-12.js'; // v3.37.0

// v3.37.0 Migration APIs
...
// v3.37.0 Runtime Validation APIs
...
// v3.37.0 RC Issue Fix, Blocker Closure, and Regression Patch Pass
...
class AnimXCore {
  constructor() {
    ...
    bindSvgAnimX(this);
    // bindCMSApi(this); - Replaced by v3.37.0
  ...
  // Advanced Interaction APIs (v3.37.0)
  ...
  // Advanced UI Feedback APIs (v3.37.0)
  ...
  // Advanced Background Motion APIs (v3.37.0)
  ...
  // Advanced SVG Motion APIs (v3.37.0)
  ...
  // Advanced Transition APIs (v3.37.0)
  ...
  getRecipe(id) { return getRecipe(id); }
  getRecipes() { return getRecipes(); }
  getCMSRecipes() { return getCMSRecipes(); }
  refreshCMS(root) { return refreshCMS(root); }
  observeCMS(root, config) { return observeCMS(root, config); }
  disconnectCMS() { return disconnectCMS(); }
  validateCMSRecipe(recipe) { return validateCMSRecipe(recipe); }
  exportCMSRecipe(recipeId, format) { return exportCMSRecipe(recipeId, format); }
  cmsAudit() { return cmsAudit(); }
  destroyCMS() { return destroyCMS(); }
  
  // Advanced Animation Marketplace-Ready Preset Packaging (v3.37.0)
  packs() { return packs(); }
  registerPack(pack) { return registerPack(pack); }
  getPack(id) { return getPack(id); }
  getPacks() { return getPacks(); }
  getPackCatalog() { return getPackCatalog(); }
  importPack(packJson, options) { return importPack(packJson, options); }
  exportPack(packId, options) { return exportPack(packId, options); }
  validatePack(pack) { return validatePack(pack); }
  auditPack(pack) { return auditPack(pack); }
  checkPackCompatibility(pack) { return checkPackCompatibility(pack); }
  destroyPacks() { return destroyPacks(); }
  
  // Final Animation Catalog Audit (v3.37.0)
  finalAudit() { return finalAudit(); }
  
  // Compat System (v3.37.0)
  supports(feature) { return compatApi.supports(feature); }
  getFeatureSupport() { return compatApi.getFeatureSupport(); }
  getBrowserInfo() { return compatApi.getBrowserInfo(); }
  getFallback(presetName) { return compatApi.getFallback(presetName); }
  registerFallback(presetName, fallbackDef) { return compatApi.registerFallback(presetName, fallbackDef); }
  validateFallbacks() { return compatApi.validateFallbacks(); }
  compatReport() { return compatApi.compatReport(); }
  ...
  // Old CMS APIs replaced by v3.37.0 implementation above
  ...
  animate(selector, animationInput, options = {}) {
    ...
    const drivers = elements.map(el => {
      ...
      if (typeof animationInput === 'string') {
        const features = compatApi.getFeatureSupport();
        const isReduced = compatApi.supports('reducedMotion') || (typeof window !== 'undefined' && window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches);
        const resolvedPresetName = compatApi.resolveFallback ? compatApi.resolveFallback(animationInput, features, isReduced) : animationInput;
        
        const preset = getPreset(resolvedPresetName);
        if (preset) {
